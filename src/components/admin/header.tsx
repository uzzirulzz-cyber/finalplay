    ...
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-white/5 bg-[#070b18]/80 px-4 backdrop-blur-xl lg:px-6">
      ...
      <div className="flex flex-1 items-center justify-end gap-2 md:flex-none">
        ...
        <div className="relative">
          ...
          >
            <div className="grid h-9 w-9 place-items-center overflow-hidden rounded-full border border-yellow-400/40 bg-slate-950 shadow-lg shadow-yellow-500/20">
              <img src={user?.avatar || '/playbeat-logo.png'} alt="Super admin profile" className="h-full w-full object-cover" />
            </div>
            <div className="hidden text-left leading-tight md:block">
              <div className="text-sm font-semibold text-white">{user?.name || 'PlayBeat Admin'}</div>
            ...
            <>
              ...
              <div className="absolute right-0 z-50 mt-2 w-56 overflow-hidden rounded-xl border border-white/10 bg-[#0f172a]/95 pb-shadow-lg backdrop-blur-xl">
                <div className="border-b border-white/5 p-3">
                  <div className="flex items-center gap-2">
                    <img src={user?.avatar || '/playbeat-logo.png'} alt="Super admin profile" className="h-8 w-8 rounded-full border border-yellow-400/30 bg-slate-950 object-cover" />
                    <div>
                      <div className="text-sm font-semibold text-white">{user?.name || 'PlayBeat Admin'}</div>
                      <div className="text-[11px] text-slate-400">{user?.email || 'admin@playbeat.digital'}</div>
                    </div>
                  </div>
                </div>
                <div className="py-1">
                  ...
                  <button onClick={logout} className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs text-red-400 transition hover:bg-red-500/5">
